...
class TennisRobot(Go2ArxRobot):
    ...
    def _init_buffers(self):
        ...
        self.prev_ekf_error = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.strike_gate = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.strike_success_ema = torch.tensor(0.0, dtype=torch.float, device=self.device)
        self.warmup_active = True
        self.current_delta_t = self.cfg.strike.delta_t_start
        self.arm_power_ema = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.last_arm_power = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.last_arm_power_penalty = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.last_torque_limit_penalty = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        self.last_ekf_improvement = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
    ...
    def reset_idx(self, env_ids):
        ...
        self.prev_ekf_error[env_ids] = 0.0
        self.arm_power_ema[env_ids] = 0.0

    def _get_strike_metrics(self):
        ball_dist = torch.norm(self.ee_pos - self.ball_pos, dim=-1)

        target_vec = torch.tensor(self.cfg.strike.target_return_vec, device=self.device, dtype=torch.float).unsqueeze(0)
        target_vec = F.normalize(target_vec.repeat(self.num_envs, 1), dim=-1)

        racket_normal = quat_apply(self.ee_orn, to_torch([0.0, 0.0, 1.0], device=self.device).repeat(self.num_envs, 1))
        rot_align = torch.sum(F.normalize(racket_normal, dim=-1) * target_vec, dim=-1)
        rot_align = torch.clamp(rot_align, -1.0, 1.0)

        ee_speed = torch.norm(self.ee_vel[:, :3], dim=-1)
        return ball_dist, rot_align, ee_speed

    def _reward_strike_pos(self):
        ball_dist, _, _ = self._get_strike_metrics()
        pos_rew = torch.exp(-(ball_dist ** 2) / self.cfg.strike.pos_sigma)
        return pos_rew * self.strike_gate.float()

    def _reward_strike_rot(self):
        _, rot_align, _ = self._get_strike_metrics()
        rot_rew = 0.5 * (rot_align + 1.0)
        return rot_rew * self.strike_gate.float()

    def _reward_strike_vel(self):
        _, _, ee_speed = self._get_strike_metrics()
        return (ee_speed > self.cfg.strike.vel_threshold).float() * self.strike_gate.float()

    def _reward_ekf_improvement(self):
        gt_ball_pos_body, _ = self._get_ball_state_body_frame()
        current_error = torch.norm(self.ekf.state[:, 0:3] - gt_ball_pos_body, dim=-1)
        improvement = self.prev_ekf_error - current_error
        self.last_ekf_improvement = improvement
        return improvement

    def _reward_fov_visibility(self):
        return self.visibility_flag.squeeze(-1)

    def _reward_warmup_track(self):
        if not self.warmup_active:
            return torch.zeros(self.num_envs, device=self.device)
        ball_dist, _, _ = self._get_strike_metrics()
        return torch.exp(-(ball_dist ** 2) / self.cfg.strike.warmup_dist_sigma)

    def _reward_base_accel(self):
        base_accel = (self.root_states[:, 7:10] - self.last_root_vel[:, 0:3]) / self.dt
        return torch.norm(base_accel, dim=-1)

    def _reward_base_dof_vel(self):
        return torch.mean(torch.abs(self.dof_vel), dim=-1)

    def _reward_arm_power_penalty(self):
        arm_power = torch.sum(torch.abs(self.torques[:, self.arm_indices] * self.dof_vel[:, self.arm_indices]), dim=-1)
        tau = min(1.0, self.dt / max(self.cfg.safety.arm_power_window_s, 1e-3))
        self.arm_power_ema = (1.0 - tau) * self.arm_power_ema + tau * arm_power
        overload = torch.relu(self.arm_power_ema - self.cfg.safety.arm_power_limit) / self.cfg.safety.arm_power_limit
        self.last_arm_power = self.arm_power_ema
        self.last_arm_power_penalty = overload
        return overload

    def _reward_torque_limit_penalty(self):
        torque_ratio = torch.abs(self.torques[:, self.arm_indices]) / (self.torque_limits[self.arm_indices].unsqueeze(0) + 1e-6)
        penalty = torch.mean(torch.relu(torque_ratio - self.cfg.safety.torque_limit_ratio), dim=-1)
        self.last_torque_limit_penalty = penalty
        return penalty

    def compute_reward(self):
        current_time = self.episode_length_buf.float() * self.dt
        prev_ekf_error = self.prev_ekf_error.clone()
        progress = min(1.0, (self.common_step_counter / max(1.0, self.max_episode_length)) / max(1, self.cfg.strike.curriculum_iters))
        self.current_delta_t = self.cfg.strike.delta_t_start + (self.cfg.strike.delta_t_end - self.cfg.strike.delta_t_start) * progress
        self.strike_gate = torch.abs(current_time - self.time_to_intercept) < self.current_delta_t

        super().compute_reward()

        gt_ball_pos_body, _ = self._get_ball_state_body_frame()
        current_error = torch.norm(self.ekf.state[:, 0:3] - gt_ball_pos_body, dim=-1)
        self.last_ekf_improvement = prev_ekf_error - current_error
        self.prev_ekf_error[:] = current_error

        ball_dist, _, ee_speed = self._get_strike_metrics()
        strike_success = self.strike_gate & (ball_dist < self.cfg.strike.success_dist) & (ee_speed > self.cfg.strike.vel_threshold)
        self.strike_success_ema = 0.995 * self.strike_success_ema + 0.005 * strike_success.float().mean()

        if self.warmup_active:
            warmup_step_budget = self.cfg.strike.warmup_iters * self.max_episode_length
            if (self.common_step_counter > warmup_step_budget) or (self.strike_success_ema.item() >= self.cfg.strike.warmup_success_threshold):
                self.warmup_active = False

        episode_extras = self.extras.setdefault("episode", {})
        episode_extras["strike_gate_rate"] = self.strike_gate.float().mean()
        episode_extras["strike_delta_t"] = torch.tensor(self.current_delta_t, device=self.device)
        episode_extras["strike_success_ema"] = self.strike_success_ema
        episode_extras["warmup_active"] = torch.tensor(float(self.warmup_active), device=self.device)
        episode_extras["reward_strike_pos"] = self._reward_strike_pos().mean()
        episode_extras["reward_strike_rot"] = self._reward_strike_rot().mean()
        episode_extras["reward_strike_vel"] = self._reward_strike_vel().mean()
        episode_extras["reward_ekf_improvement"] = self.last_ekf_improvement.mean()
        episode_extras["reward_fov"] = self._reward_fov_visibility().mean()
        episode_extras["arm_power"] = self.last_arm_power.mean()
        episode_extras["arm_power_penalty"] = self.last_arm_power_penalty.mean()
        episode_extras["torque_limit_penalty"] = self.last_torque_limit_penalty.mean()
